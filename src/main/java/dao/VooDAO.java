...
public class VooDAO {
	...
	public Voos buscarVoo(String nmrVoo) throws ParseException {
		Voos voos;
		try (PreparedStatement stm = connection.prepareStatement("SELECT * FROM voos WHERE NMRVOO = ?")) {
			stm.setString(1, nmrVoo);
			stm.execute();
			try(ResultSet rst = stm.getResultSet()){
				if(rst.next()){
					return voos = new Voos(rst.getInt(1), rst.getString(2), rst.getFloat(3), rst.getString(4), rst.getString(5), rst.getString(6), rst.getInt(7));
				}else{
					return null;
				}
			}
